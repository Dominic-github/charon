...
class WatchRecordScannerTest extends TestCase
{
    ...
    public function scanDeletedDirectoryViaWatch(): void
    {
        ...
        File::copyDirectory(
            ...
            $this->mediaPath . '/subdir_copy'
        );

        File::deleteDirectory($this->mediaPath . '/subdir_copy');

        $this->scanner->scan(new InotifyWatchRecord("MOVED_FROM,ISDIR $this->mediaPath/subdir_copy"), $config);

        $this->assertDatabaseMissing(Song::class, ['path' => $this->path('/subdir_copy/sic.mp3')]);
        $this->assertDatabaseMissing(Song::class, ['path' => $this->path('/subdir_copy/no-name.mp3')]);
        $this->assertDatabaseMissing(Song::class, ['path' => $this->path('/subdir_copy/back-in-black.mp3')]);
    }
}
