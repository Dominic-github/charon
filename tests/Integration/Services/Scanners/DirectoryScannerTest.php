...
class DirectoryScannerTest extends TestCase
{
    ...
    public function scan(): void
    {
        ...
        $this->assertDatabaseHas(Album::class, ['name' => 'Charon Testing Vol. 1']);

        // Albums and artists should be correctly linked
        /** @var Album $album */
        $album = Album::query()->where('name', 'Charon Testing Vol. 1')->first();
        self::assertSame('Charon', $album->artist->name);
        ...
        self::assertSame('Charon', $song->album_artist->name);
    ...
    public function forceScanResetsData(): void
    {
        ...
        $this->scanner->scan($this->mediaPath, ScanConfiguration::make(owner: $owner, force: true));
    ...
    public function hiddenFilesAndFoldersAreIgnoredIfConfiguredSo(): void
    {
        ...
        config(['charon.ignore_dot_files' => true]);
        $this->scanner->scan($this->mediaPath, $config);
    ...
    public function hiddenFilesAndFoldersAreScannedIfConfiguredSo(): void
    {
        ...
        config(['charon.ignore_dot_files' => false]);
        $this->scanner->scan($this->mediaPath, $config);
