...
class S3CompatibleStorageTest extends TestCase
{
    ...
    public function storeUploadedFile(): void
    {
        ...
        Storage::disk('s3')->assertExists(Str::after($reference->location, 's3://charon/')); // 'charon' is the bucket name

        self::assertSame("s3://charon/{$user->id}__random__full.mp3", $reference->location);
    ...
    public function undoUpload(): void
    {
        ...
        $reference = UploadReference::make(
            location: 's3://charon/123__random__song.mp3', // 'charon' is the bucket name
    ...
    public function getPresignedUrl(): void
    {
        ...
        $url = $this->service->getPresignedUrl(Str::after($reference->location, 's3://charon/'));

        self::assertStringContainsString(Str::after($reference->location, 's3://charon/'), $url);
    }
}
