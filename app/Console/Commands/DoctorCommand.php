...
{
    protected $signature = 'charon:doctor';
    protected $description = 'Check Charon setup';
    ...
    public function handle(): int
    {
        ...
        $this->components->alert('Checking Charon setup...');
        $this->line('');
        ...
        if ($this->errors) {
            ...
            $this->output->success('Your Charon setup should be good to go!');
        }
    ...
    {
        $this->components->error('There are errors in your Charon setup. Charon will not work properly.');

        if (File::isWritable(base_path('storage/logs/laravel.log'))) {
            /** @var Throwable $error */
            foreach ($this->errors as $error) {
                Log::error('[CHARON.DOCTOR] ' . $error->getMessage(), ['error' => $error]);
            }
    ...
    private function checkScheduler(): void
    {
        ...
        if (InstallSchedulerCommand::schedulerInstalled($crontab)) {
            $this->reportSuccess('Charon scheduler status', 'Installed');
        } else {
            $this->reportWarning('Charon scheduler status', 'Not installed');
        }
    }
    ...
    {
        $this->reportInfo('Streaming method', config('charon.streaming.method'));
    }
    ...
    {
        $ffmpegPath = config('charon.streaming.ffmpeg_path');
