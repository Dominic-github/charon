...
class Folder extends Model
{
    ...
    {
        // An uploads folder has a format of __CHARON_UPLOADS_$<id>__ and is a child of the root folder
        // (i.e., it has no parent).
        return Attribute::get(
            fn() => !$this->parent_id && preg_match('/^__CHARON_UPLOADS_\$\d+__$/', $this->name) === 1
        );
    }
    ...
    protected function uploaderId(): Attribute
    {
        return Attribute::get(function () {
            ...
            $matches = [];
            preg_match('/^__CHARON_UPLOADS_\$(\d+)__$/', $this->name, $matches);
