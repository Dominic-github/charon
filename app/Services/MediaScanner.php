...
class MediaScanner
{
    ...
    private function gatherFiles(string $path): array
    {
        return iterator_to_array(
            $this->finder->create()
                ...
                ->ignoreDotFiles((bool) config('charon.ignore_dot_files')) // https://github.com/dominic-github/charon/issues/450
                ->files()
                ->followLinks()
    ...
    private function setSystemRequirements(): void
    {
        if (!app()->runningInConsole()) {
            set_time_limit(config('charon.sync.timeout'));
        }

        if (config('charon.memory_limit')) {
            ini_set('memory_limit', config('charon.memory_limit') . 'M');
        }
    }
