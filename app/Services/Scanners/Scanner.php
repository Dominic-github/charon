...
abstract class Scanner
{
    ...
    protected function gatherFiles(string $path): Finder
    {
        ...
        return $this->finder::create()
            ...
            ->ignoreDotFiles((bool) config('charon.ignore_dot_files'))
            ->files()
            ->followLinks()
            ->name($nameRegex)
            ->in($path);
    ...
    protected static function setSystemRequirements(): void
    {
        if (!app()->runningInConsole()) {
            set_time_limit(config('charon.scan.timeout'));
        }

        if (config('charon.scan.memory_limit')) {
            ini_set('memory_limit', config('charon.scan.memory_limit') . 'M');
        }
    }
}
