...
class Streamer
{
    ...
    private static function shouldTranscode(Song $song): bool
    {
        ...
        if (
            ...
            && config('charon.streaming.transcode_flac')
        ) {
            return true;
        }

        return in_array($song->mime_type, config('charon.streaming.transcode_required_mime_types', []), true);
    }

    private static function hasValidFfmpegInstallation(): bool
    {
        return app()->runningUnitTests() || is_executable(config('charon.streaming.ffmpeg_path'));
    }
}
