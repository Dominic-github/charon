...
new class extends UnitTestCase {
  protected test () {
    ...
    it('shows About Charon model', async () => {
      const emitMock = this.mock(eventBus, 'emit')
      this.renderComponent()

      await this.user.click(screen.getByRole('button', { name: 'About Charon' }))

      expect(emitMock).toHaveBeenCalledWith('MODAL_SHOW_ABOUT_CHARON')
    })
